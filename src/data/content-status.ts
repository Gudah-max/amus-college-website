...
export const contentStatus: Record<string, { status: ContentStatus; note?: string }> = {
  ...
  oLevelAdmissionsRequirements: { status: 'working-source' }, aLevelEligibility: { status: 'awaiting-confirmation' }, feesRoute: { status: 'working-source' }, scholarshipDetails: { status: 'awaiting-confirmation' }, intakeDates: { status: 'awaiting-confirmation' }, officeHours: { status: 'approved' }, contactDetails: { status: 'working-source' }, formPrivacyPolicy: { status: 'awaiting-confirmation' }, policyReview: { status: 'awaiting-confirmation', note: 'Privacy and cookie policy wording requires school confirmation.' },
};
